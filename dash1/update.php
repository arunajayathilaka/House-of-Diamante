	...
	if(isset($_POST['cName'])&& ($_POST['cName']!=null) ){
       ...
       if(isset($_POST['confirmpassword'])&& isset($_POST['currentpassword'])&&isset($_POST['newpassword'])){
		if(($_POST['confirmpassword']!=null)&&($_POST['currentpassword']!=null)&&($_POST['newpassword']!=null) ){
			$curr = mysql_query("SELECT vendor_password FROM vendor WHERE  	vendor_email = '{$_SESSION['login_user']}'"); 
			if(($_POST['currentpassword'])== mysql_fetch_array($curr)["vendor_password"]){
        ...
        // $filepath = "img/vendor images/".$_FILES['file']['name'];
         move_uploaded_file($_FILES['image']['tmp_name'], "img/vendor images/".$_FILES['image']['name']);
         $imgchanege = "img/vendor images/{$_FILES['image']['name']}";
         echo "im".$_FILES['image']['name']."</br>";
